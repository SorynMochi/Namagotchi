...
### Structural batch — 2026-07-10 — Chat floating emoji picker mounted-state consolidation

- Scope: one structural consolidation batch inside the `Chat / emoji / profile` family. Touched selectors: `body > .emoji-picker-floating`, `body > .emoji-picker-floating.hidden`, and `body > .emoji-picker-floating:not(.hidden)`.
- Selector-family choice: selected `body > .emoji-picker-floating` from `docs/css-cleanup-remaining-work.md` because it is the next high-value non-`:root` structural family with localized declarations and a deterministic cascade-equivalence check. The larger Wardrobe modal action family remains riskier because its repair blocks mix absolute positioning, grid placement, mobile modal behavior, and `!important` source-order repairs that require live Wardrobe modal visual coverage before a safe consolidation.
- Cascade context recorded: all changed occurrences were top-level global rules in `web/styles.css`; no `@media`, `@supports`, `@layer`, `@scope`, or container context was changed.
- Consolidation performed: removed the superseded pre-render performance block and kept the still-effective `body > .emoji-picker-floating:not(.hidden)` declarations beside the permanently mounted emoji tray rules. The surviving later base and state rules preserve final `display`, `visibility`, `opacity`, `pointer-events`, `will-change`, transform, sizing, and containment behavior for the floating picker, hidden state, not-hidden state, and open state.
- Declarations and lines removed: 7 net CSS declarations and 14 net stylesheet lines.
- Line count before/after: `10,707` lines before; `10,693` lines after.
- Verification: same-selector final declaration snapshot matched the pre-change stylesheet for `body > .emoji-picker-floating`, `body > .emoji-picker-floating.hidden`, `body > .emoji-picker-floating:not(.hidden)`, and `body > .emoji-picker-floating.is-open`; CSS brace parse passed; `go test ./...` passed.
- Visual coverage: no browser automation, screenshot tooling, or Chromium binary is installed in this repository/container. Manual visual comparison is still recommended for chat emoji picker hidden/open states at desktop and mobile widths.
