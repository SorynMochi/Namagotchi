...
func (s *Server) Routes() http.Handler {
	...
	mux.HandleFunc("/api/dev/reset-chain", s.requireDev(s.requireDevUnlock(s.withDevAudit("reset-chain", s.requireCSRF(s.HandleDevResetChain)))))
	mux.HandleFunc("/api/dev/reset-max-chain", s.requireDev(s.requireDevUnlock(s.withDevAudit("reset-max-chain", s.requireCSRF(s.HandleDevResetMaxChain)))))
	...
	mux.HandleFunc("/api/dev/clear-wardrobe", s.requireDev(s.requireDevUnlock(s.withDevAudit("clear-wardrobe", s.requireCSRF(s.HandleDevClearWardrobe)))))
