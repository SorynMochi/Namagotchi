...
func (s *Server) Routes() http.Handler {
	...
	mux.HandleFunc("/api/dev/seed-player", s.requireDev(s.withDevAudit("seed-player", s.HandleSeedDevPlayer)))
	mux.HandleFunc("/api/dev/force-tick", s.requireDev(s.withDevAudit("force-tick", s.HandleForceTick)))
	mux.HandleFunc("/api/dev/reset-playdeck-streak", s.requireDev(s.withDevAudit("reset-playdeck-streak", s.HandleResetPlaydeckStreak)))
	mux.HandleFunc("/api/dev/rewind-care-decay", s.requireDev(s.withDevAudit("rewind-care-decay", s.HandleRewindCareDecay)))
	mux.HandleFunc("/api/dev/spawn-wardrobe-item", s.requireDev(s.withDevAudit("spawn-wardrobe-item", s.HandleSpawnDevWardrobeItem)))
	mux.HandleFunc("/api/dev/audit-logs", s.requireDev(s.HandleDevAuditLogs))
